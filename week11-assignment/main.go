...
func getNewBook(c *gin.Context) {
 ...
 func getDiscountBook(c *gin.Context) {
    var book Book

    // QueryRow ใช้เมื่อคาดว่าจะได้ผลลัพธ์ 0 หรือ 1 แถว
    err := db.QueryRow("SELECT id, title, author, discount FROM books WHERE discount > 0").
        Scan(&book.ID, &book.Title, &book.Author, &book.Discount)
...
// @Description Get details of a book by its category
// @Tags Books
// @Produce  json
// @Param   category   path   string  true  "Book category"
// @Success 200 {object} Book
// @Failure 404 {object} ErrorResponse
...
func getBook(c *gin.Context) {
    ...
    // QueryRow ใช้เมื่อคาดว่าจะได้ผลลัพธ์ 0 หรือ 1 แถว
    err := db.QueryRow("SELECT id, title, author, category FROM books WHERE category = $1", category).
...
func getFeaturedBooks(c *gin.Context) {
	...
	var err error
	// ลูกค้าถาม "มีหนังสืออะไรบ้าง"
	rows, err = db.Query("SELECT id, title, author, isbn, year, price, rating FROM books order by rating desc limit 5")
	if err != nil {
	    c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
	    return
	}
	...
	for rows.Next() {
	    var book Book
	    err := rows.Scan(&book.ID, &book.Title, &book.Author, &book.ISBN, &book.Year, &book.Price, &book.Rating)
	    if err != nil {
		   // handle error
	    }
	    books = append(books, book)
...
func SearchKeyword(c *gin.Context) {
	...
	// ถ้าไม่ได้ส่ง keyword มาเลย
	if keyword == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "missing keyword parameter"})
		return
	}

	// สร้าง query ให้ค้นหาจากทุก column ที่ต้องการ
	query := `
		SELECT id, title, author, category, isbn, year, price, created_at, updated_at
		FROM books
		WHERE 
			...
			category ILIKE '%' || $1 || '%' OR
			isbn ILIKE '%' || $1 || '%' OR
			CAST(year AS TEXT) ILIKE '%' || $1 || '%' OR
			CAST(price AS TEXT) ILIKE '%' || $1 || '%' OR
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&book.ISBN, &book.Year, &book.Price, &book.CreatedAt, &book.UpdatedAt,
		)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
...
func getCategory(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "ไม่สามารถดึง categories ได้"})
		return
	}
	defer rows.Close()
...
func main(){
	...
	api := r.Group("/api/v1")
	{
		...
		//กรองตามหมวดหมู่
		api.GET("/books/category/:category", getBook)
	 	//api.GET("/books/:id", getBook)
	 	api.POST("/books", createBook)
	 	api.PUT("/books/:id", updateBook)
	 	api.DELETE("/books/:id", deleteBook)

		api.GET("/categories", getCategory)
		api.GET("books/search",SearchKeyword)
		api.GET("books/featured", getFeaturedBooks)
		api.GET("/books/new", getNewBook)
		api.GET("books/discounted", getDiscountBook)
	 }
